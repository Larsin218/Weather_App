...
function updateForecastDetails(data) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    forecast[i].querySelector(".dayTemp").textContent = `Temp: ${
      data.list[i * 8].main.temp
    }°F`;
    forecast[i].querySelector(".dayWind").textContent = `Wind: ${
      data.list[i * 8].wind.speed
    } m/s`;
    forecast[i].querySelector(".dayHumidity").textContent = `Humidity: ${
...
function saveHistory(cityName) {
    ...
    localStorage.setItem(`city${i}`, JSON.stringify(cityName));
}
...
function loadHistory() {
    ...
    historyP.textContent = city;
    historyList.insertBefore(historyP, historyList.firstChild);
    console.log('i', i);
    console.log('key', key);
